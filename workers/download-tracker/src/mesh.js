...
export const MESH_PRODUCT = "4dmap";
...
export function meshPointer() {
  return {
    ...
    origin: RUNTIME + MESH_PATH,
    note: "PROXY to aziel-runtime /v1/mesh/* via AZIEL_RUNTIME. Not a local op. Not AnonBroadcast. Not AZMail's product-local ring. 4DMap remains an inspection coordinate frame (not a truth engine, not Lumen, not GIS, not a Node Gate). Full node process is local qnm-node/. QNS-CD-1.0 photon QNS1 packet transfer is a hub cite / Worker mesh cross-map only (local qnsd in qnm-node; runtime cite in aziel-runtime; AZInterface pair custody). No public qnsd proxy. " + MESH_NOTE,
...
export function meshOpenApiPaths() {
  ...
  for (const route of MESH_PROXY_ROUTES) {
    ...
    for (const method of route.methods) {
      entry[method] = {
        operationId: "fourdmap_mesh_" + route.op + (method === "head" ? "_head" : "") + "_proxy",
...
async function originFetch(env, pathAndQuery, init, request) {
  ...
  headers.set("X-Aziel-Runtime-Via", "4dmap-download-tracker");

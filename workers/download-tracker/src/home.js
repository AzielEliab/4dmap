...
export const SEO_DESCRIPTION =
  "4DMap by Aziel Eliab: a four-axis inspection coordinate frame. T clock, Δ interval, Γ trajectory, Π pattern. 4DM-CARD receipts. Apache-2.0.";
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .wrap { width: 100%; max-width: 58rem; margin: 0 auto; padding: 1.15rem 1rem 2.75rem; }
  a { color: var(--link); }
  a:focus-visible, button:focus-visible, input:focus-visible, select:focus-visible, textarea:focus-visible, summary:focus-visible {
    outline: 2px solid var(--focus);
    outline-offset: 2px;
  }
  .skip { position: absolute; left: .75rem; top: 0; transform: translateY(-120%); background: var(--btn-bg); color: var(--btn-fg); padding: .45rem .75rem; text-decoration: none; border-radius: 8px; z-index: 5; }
  .skip:focus { transform: none; }
  .hero { margin: 0 0 1rem; }
  .brandrow { display: flex; align-items: center; gap: 12px; flex-wrap: wrap; margin: 0 0 12px; }
  ...
  .tag { margin: 0 0 1rem; color: var(--muted); font-size: .78rem; letter-spacing: .06em; text-transform: uppercase; }
  ...
  a.btn.block.primary { display: block; width: 100%; max-width: 40rem; margin: 0 0 .7rem; padding: 1.05rem 1.2rem; border: 1px solid transparent; border-radius: 10px; background: var(--btn-bg); color: var(--btn-fg); text-align: center; text-decoration: none; font: 700 1.35rem/1.1 ui-monospace, Menlo, Consolas, monospace; letter-spacing: .03em; cursor: pointer; }
  a.btn.block.primary:hover { filter: brightness(1.08); }
  .asset-note { color: var(--muted); font-size: .92rem; margin: 0 0 1rem; max-width: 40rem; }
  .features { display: grid; grid-template-columns: 1fr; gap: .45rem .9rem; margin: 0 0 1rem; padding: 0; list-style: none; max-width: 46rem; }
  ...
  button.btn.install { display: block; width: 100%; max-width: 40rem; margin: 0 0 .55rem; padding: .8rem 1rem; border-radius: 10px; border: 1px solid var(--line); background: transparent; color: var(--ink); text-align: center; font: 650 1rem/1.2 ui-monospace, Menlo, Consolas, monospace; cursor: pointer; }
  .card, .cite { border: 1px solid var(--line); border-radius: 14px; padding: 1.05rem 1.1rem 1.15rem; background: var(--surface); margin: 0 0 1rem; }
  .nums { display: grid; grid-template-columns: 1fr 1fr; gap: .8rem; margin: 0 0 .8rem; }
  ...
  .meta, .iso, .kid, .note { color: var(--muted); font-size: .92rem; overflow-wrap: anywhere; }
  .scope { border: 1px solid var(--line); background: var(--surface); color: var(--ink); padding: .9rem 1rem; border-radius: 12px; margin: 0 0 1rem; font-size: .92rem; }
  details.card summary { cursor: pointer; font-weight: 650; }
  .pipe, pre { white-space: pre-wrap; overflow-wrap: anywhere; max-width: 100%; font: 12px/1.45 ui-monospace, Menlo, Consolas, monospace; color: var(--ink); border: 1px solid var(--line); padding: .75rem .9rem; border-radius: 8px; margin: .7rem 0 0; background: var(--code-bg); }
  #meshStrip { border: 1px solid var(--line); border-radius: 12px; padding: .85rem 1rem; background: var(--surface); margin: 0 0 1rem; display: flex; flex-wrap: wrap; align-items: center; gap: .7rem 1rem; font-size: .88rem; color: var(--muted); }
  #meshStrip .live b, #meshStrip .rollup b { color: var(--gold); }
  #meshStrip .mesh-actions { display: flex; flex-wrap: wrap; gap: .45rem; align-items: center; width: 100%; }
  #meshStrip button { font: 700 .78rem/1 ui-monospace, Menlo, Consolas, monospace; min-height: 2rem; padding: 0 .75rem; border-radius: 8px; background: var(--input-bg); color: var(--ink); border: 1px solid var(--line); cursor: pointer; }
  #meshStrip input { width: min(12rem, 100%); max-width: 100%; padding: .4rem .55rem; border: 1px solid var(--line); border-radius: 8px; background: var(--input-bg); color: var(--ink); font: inherit; }
  h2 { font-size: 1.12rem; margin: 0 0 .45rem; letter-spacing: .02em; }
  .axes { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: .55rem; }
  .axis { border: 1px solid var(--line); border-radius: 10px; padding: .65rem; min-height: 8rem; min-width: 0; background: var(--plot); color: #f4efe4; }
  .axis h2 { margin: 0 0 .45rem; color: #e6c35c; font-size: .95rem; }
  .tick { border: 1px solid #7d756a; border-radius: 8px; padding: .4rem .45rem; margin: .3rem 0; font-size: .78rem; overflow-wrap: anywhere; }
  .tick code { color: #e6c35c; }
  label { display: block; color: var(--muted); font-size: .85rem; margin: .35rem 0 .15rem; }
  input, select, textarea { width: 100%; max-width: 100%; padding: .5rem .6rem; border: 1px solid var(--line); border-radius: 8px; background: var(--input-bg); color: var(--ink); font: inherit; }
  textarea { min-height: 6rem; font: 12px/1.4 ui-monospace, Menlo, Consolas, monospace; }
  input::placeholder, textarea::placeholder { color: var(--muted); opacity: 1; }
  .inline { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr) auto; gap: .45rem; align-items: end; }
  .inline4 { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr) minmax(0, 1fr) auto; gap: .45rem; align-items: end; }
  button.gold { width: auto; font: 700 .88rem/1.1 ui-monospace, Menlo, Consolas, monospace; padding: .72rem .9rem; border-radius: 9px; border: 1px solid transparent; background: #c9a227; color: var(--on-gold); cursor: pointer; }
  .toolrow { display: flex; flex-wrap: wrap; gap: .45rem; margin: .6rem 0; }
  .receipt { border: 1px solid var(--line); border-radius: 8px; padding: .45rem .55rem; margin: .35rem 0; font-size: .78rem; overflow-wrap: anywhere; }
  .receipt b { color: var(--gold); }
  .plot { width: 100%; max-width: 100%; height: 220px; border: 1px solid var(--line); border-radius: 10px; background: var(--plot); }
  .scores { display: grid; grid-template-columns: 1fr 1fr; gap: .55rem; margin: .6rem 0; }
  .scorebox { border: 1px solid var(--line); border-radius: 8px; padding: .55rem .65rem; font-size: .82rem; min-width: 0; overflow-wrap: anywhere; }
  .scorebox b { color: var(--gold); }
  .mock { color: var(--muted); }
  .real { color: var(--ok); }
  footer.quiet { color: var(--muted); font-size: .9rem; margin: .4rem 0 0; padding-top: .2rem; }
  ...
  @media (max-width: 720px) {
    .inline, .inline4 { grid-template-columns: minmax(0, 1fr); }
    .axes { grid-template-columns: minmax(0, 1fr) minmax(0, 1fr); }
    a.btn.block.primary, button.btn.install { max-width: none; }
    button.gold { width: 100%; }
  }
  @media (max-width: 420px) {
    .axes, .scores, .nums { grid-template-columns: minmax(0, 1fr); }
...
<div class="wrap">
  <header class="hero">
    ...
    <h1>4DMap</h1>
    <p class="motto">Four-axis inspection coordinate frame.</p>
    <p class="lede">v0.3.0 software by Aziel Eliab. T Clock, Δ Interval, Γ Trajectory, and Π Pattern.</p>
    <a class="btn block primary" id="downloadBtn" href="/download?asset=${assetName}" aria-describedby="downloadNote">Download</a>
    <p class="asset-note" id="downloadNote">${n} downloads · ${assetName} · counted on this Worker for every branch and fork</p>
    <p class="tag">v0.3.0 · 4DM-WP-1.0 · Plain · Apache-2.0 · inspection frame · Growth-ON</p>
    <ul class="features">
      <li>T Clock, Δ Interval, Γ Trajectory, and Π Pattern</li>
      <li>4DM-CARD receipts with fail-closed SHA-256. Forks are kept.</li>
      <li>The board on this page holds the cards for this visit.</li>
    </ul>
    <button type="button" class="btn install" id="install-btn">Copy install command</button>
    <pre id="install-cmd">${INSTALL_LINE}</pre>
  </header>

  <section class="card" id="install" aria-label="Download counts">
    <div class="nums">
      <p class="count">${v}<span>Views</span></p>
      <p class="count">${n}<span>Downloads</span></p>
    </div>
    <p class="kid">Download saves the gzip and the Downloads number goes up. Copy install command copies a Terminal command. After it finishes: <code>4dmap ui</code> then open http://127.0.0.1:8844.</p>
    <p class="iso">Isolated counter: Worker <code>4dmap-download-tracker</code>, project <code>4dmap</code>, KV <code>4DMAP_DOWNLOADS</code>. <code>/v1</code> does not increment. The hosted API keeps no map.</p>
  ...
  <details class="card">
    <summary>Pipeline placement</summary>
    <p class="note">4DMap sits in the Internal Domain Layer as a read-side inspection frame (domains_are_doors:false), not a hop gate.</p>
...
${escapeHtml(PIPELINE_NOTE)}</pre>
  ...
  <p class="scope" role="note">${escapeHtml(LIMITATION)}</p>
  <footer class="quiet">
    <p>Apache-2.0 · Aziel Eliab · 4DMap 0.3.0</p>
    <p>Forks are welcome. Spec 4DM-WP-1.0. Cite: Aziel Eliab. 4DMap. ${GITHUB_REPO}.</p>
    <p><a href="${GITHUB_REPO}">GitHub</a> · <a href="/stats">Stats</a> · <a href="/openapi.json">OpenAPI</a> · <a href="/v1/skill">Skill</a> · <a href="/v1/mesh">Mesh</a> · <a href="/ai">AI runtime</a> · <a href="${CATALOG}">Catalog</a> · <a href="${HOST}/cite.json">cite.json</a></p>
  </footer>
</div>
  <script>
    (function () {
      ...
      function selectCmd() {
        ...
        btn.textContent = "Command selected. Copy it, then run 4dmap ui";
      }
      if (btn) btn.addEventListener("click", function () {
        if (navigator.clipboard && navigator.clipboard.writeText) {
          navigator.clipboard.writeText(cmd).then(function () {
            btn.textContent = "Copied. Paste in Terminal, then run 4dmap ui";
          }).catch(selectCmd);
        } else {
          selectCmd();
        }
